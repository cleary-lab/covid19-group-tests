...
p1 <- ggplot(viral_loads_dat %>% filter(i %in% 1:3)) + 
  geom_point(aes(x=t,y=obs)) + 
  geom_line(aes(x=t,y=obs),size=0.25) +
  facet_wrap(~i) + scale_x_continuous(limits=c(-5,20)) +
...
plot_dat <- f_model(parTab$values)
...
p2 <- ggplot(viral_loads_dat %>% filter(i %in% 1:3)) + 
  geom_point(aes(x=t,y=obs)) + 
  geom_line(aes(x=t,y=obs),size=0.25) +
  geom_line(data=store_all %>% filter(i %in% 1:3, samp == 1), aes(x=t,y=y),col="red",size=0.5) +
  facet_wrap(~i) + scale_x_continuous(limits=c(-5,20)) +
  scale_y_continuous(limits=c(0,10),breaks=seq(0,10,by=2)) +
  geom_hline(yintercept=2, linetype="dashed") +
  ylab("log10 RNA copies/swab") +
...
p3 <- ggplot(viral_loads_dat %>% filter(i %in% 1:3)) + 
  geom_point(aes(x=t,y=obs)) + 
  geom_line(aes(x=t,y=obs),size=0.25) +
  geom_line(data=store_all %>% filter(i %in% 1:3, samp %in% 1:20), 
            aes(x=t,y=y,group=samp),col="red",size=0.5) +
  facet_wrap(~i) + scale_x_continuous(limits=c(-5,20)) +
  scale_y_continuous(limits=c(0,10),breaks=seq(0,10,by=2)) +
  geom_hline(yintercept=2, linetype="dashed") +
  ylab("log10 RNA copies/swab") +
