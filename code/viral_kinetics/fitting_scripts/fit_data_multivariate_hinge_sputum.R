...
chains <- load_mcmc_chains(paste0("~/Documents/GitHub/covid19-group-tests/code/viral_kinetics/chains/chains_",run_name), parTab, TRUE, 10, burnin=1000000,multi=TRUE)
gelman.diag(chains[[1]])
effectiveSize(chains[[1]])
chains <- load_mcmc_chains(paste0("~/Documents/GitHub/covid19-group-tests/code/viral_kinetics/chains/chains_",run_name), parTab, FALSE, 10, burnin=1000000,multi=TRUE)
#plot(chains[[2]])
...
p_sputum <- ggplot(quants) +
  ...
  scale_x_continuous(breaks=seq(0,70,by=7) - 14) +
...
prop_onset_true_sputum <- prop_onset_true
...
prop_detect <- dat_all %>% 
  ...
  geom_line(data=dat_all %>% 
              mutate(detectable = y > pars_use["lod"]) %>%
              group_by(t) %>%
              summarize(prop=sum(detectable)/n()),
            aes(x=t,y=prop),col="red",size=1) +
...
p_draws <- dat_all %>% 
  ...
  geom_line(data=mean_line,aes(x=t,y=median_line),col="darkorange",size=0.75) +
  coord_cartesian(ylim=c(0,12)) +
  scale_y_continuous(breaks=seq(0,12,by=1)) +
  scale_x_continuous(limits=c(0,50),expand=c(0,0),breaks=seq(0,50,by=5)) +
  geom_vline(xintercept=5,linetype="dashed") +
