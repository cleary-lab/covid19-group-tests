...
source("model_funcs_multivariate.R")
...
parTab <- read.csv("pars/partab_multivariate.csv",stringsAsFactors=FALSE)
...
f <- create_func_indivs_multivariate(parTab, viral_loads_dat,ver="posterior",PRIOR_FUNC=prior_func)
f(parTab$values)
res <- foreach(i=seq_along(filenames),.packages = c("lazymcmc","rethinking","extraDistr")) %dopar% {
  setwd("/Users/james/Google Drive/nCoV/pool_samples/chains_multivariate")
  ...
  x <- run_MCMC(parTab, viral_loads_dat, mcmcPars=mcmcPars1,filename=filenames[i],
                CREATE_POSTERIOR_FUNC = create_func_indivs_multivariate,PRIOR_FUNC=prior_func,ver="posterior",
                OPT_TUNING=0.2)
  ## Check convergence
...
chains <- load_mcmc_chains("~/Google Drive/nCoV/pool_samples/chains_multivariate/", parTab, FALSE, 1, burnin=200000,multi=TRUE)
plot(chains[[2]])
