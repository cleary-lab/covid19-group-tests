...
used_pars <- simulated_data$pars
used_pars <- cbind(used_pars, infection_times)
colnames(used_pars) <- c("viral_peak","t_wane","mode","tshift","incu","sd","infection_time")
used_pars <- as_tibble(used_pars)
write_csv(used_pars,paste0("sims/used_pars_",run_name,".csv"))
...
viral_loads_sparse <- Matrix(viral_loads,sparse=TRUE)
rm(viral_loads)
viral_loads_sparse <- as.data.frame(Matrix::summary(viral_loads_sparse))
viral_loads_sparse$j <- viral_loads_sparse$j - 1

#write_csv(viral_loads,paste0("seir_viral_loads_",run_name,".csv"))
data.table::fwrite(viral_loads_sparse, file = paste0("seir_viral_loads_",run_name,".csv"),sep = ",")
rm(viral_loads_sparse)

obs_dat_sparse <- Matrix(obs_dat,sparse=TRUE)
obs_dat_sparse <- as.data.frame(Matrix::summary(obs_dat_sparse))
obs_dat_sparse$j <- obs_dat_sparse$j - 1

#write_csv(obs_dat,paste0("seir_obs_viral_loads_",run_name,".csv"))
data.table::fwrite(obs_dat_sparse, file = paste0("seir_obs_viral_loads_",run_name,".csv"),sep = ",")
rm(obs_dat_sparse)

obs_vl_melted <- reshape2::melt(obs_dat[seq(1, min(1000000, nrow(obs_dat)),by=1),])
rm(obs_dat)
colnames(obs_vl_melted) <- c("i","t","obs")
