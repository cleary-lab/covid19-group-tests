...
chains <- load_mcmc_chains(paste0("~/Google Drive/nCoV/pool_samples/chains_",run_name), 
                           parTab, TRUE, 100, burnin=1000000,multi=TRUE)
gelman.diag(chains[[1]])
effectiveSize(chains[[1]])
chains <- load_mcmc_chains(paste0("~/Google Drive/nCoV/pool_samples/chains_",run_name), 
                           parTab, FALSE, 100, burnin=1000000,multi=TRUE)
plot(chains[[2]])
...
extra_rows <- dat_all %>% group_by(samp) %>% 
  ...
  group_by(samp) %>%
  expand(t_shifted=full_seq(min_t,0.1)) %>% ## Create time sequence from -30 to infection time
  ungroup() %>%
...
prop_detect_onset <- dat_all_onset %>% 
