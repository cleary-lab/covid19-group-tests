...
dat_later <- read_csv(file = "summary.resource_t0-70_t1-120.csv")
dat_later <- dat_later %>% 
  mutate(version = ifelse(`Design sample split`== 1, 
                          ifelse(`Design pools` == 1, "Simple", "Individual"),
                          "Combinatorial"))
...
p1 <- ggplot(dat) + 
  ...
  guides() +
  ggtitle("Design maximizing positive sample identification for prevalence range 0.03%-2.46%") +
  ylab("Daily reaction budget (RNA extraction, RT+qPCR)") +
  xlab("Daily sample availability") +
  theme_classic() +
...
summary_dat$version3 <- factor(summary_dat$version3, 
...
summary_dat_later <- dat_later %>% 
  ...
  mutate(min_effectiveness = ifelse(version2 == "Combinatorial", min_effectiveness, NA),
         max_effectiveness = ifelse(version2 == "Combinatorial", max_effectiveness, NA))
summary_dat_later$version3 <-  factor(summary_dat_later$version3, 
                                      levels=rev(c("Simple pools of 4","Simple pools of 8","Combinatorial q=2",
                                                   "Combinatorial q=3 or 4")))
summary_dat_later$`Stage of outbreak` <- "Prevalence 0.30%-10.1%"
...
summary_dat_all$`Stage of outbreak` <- factor(summary_dat_all$`Stage of outbreak`,
                                                 levels=rev(c("Prevalence 0.30%-10.1%","Prevalence 0.03%-2.46%")))


p3 <- ggplot(summary_dat_all, aes(y=version3)) + 
...
p_main
ggsave("fig4.png",p_main, height=5,width=8,units="in")
ggsave("fig4.pdf",p_main, height=5,width=8,units="in")
